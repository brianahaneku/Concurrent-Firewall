...
class PipelineFirewall {
    ...
    public static void main(String[] args) {
        ...
        int numHeadWorkers = 5;
        int numCacheMissWorkers = 1;
    ...
    for (int i = 0; i < numHeadWorkers; i++) {
        HeadWorker headWorkerData = new HeadWorker(done,
         headWorkerQueues[i],
          addressTable,
           totalProcessedPackets, cacheMissWorkerQueues, histogram);
        Thread headWorkerThread = new Thread(headWorkerData);
        headWorkerThreads[i] = headWorkerThread;
    }

    Dispatcher dispatcherData = new Dispatcher(done, source, headWorkerQueues);
    ...
    try {
        ...
        System.out.println("Dispatcher Interuppted");}
